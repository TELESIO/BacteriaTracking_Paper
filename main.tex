...
Segmentation  is carried out by means of threshold\cite{Shapiro:2002} method which produces a bi-partition of the pixel based on the color intensity. The value of pixel $(x,y)$ in image $g$ is given by the following where $P$ is a predicate:
${g(x,y) = \begin{cases} 1, & \mbox{if } P(f(x,y),T) \\ 0, & \mbox{otherwise }\end{cases}}$.
One drawback of using threshold method is that pixel color intensity is the only property being considerend by the bi-partition process, not any relationships between the pixels. This can easily lead to binary regions whose pixel are not contiguous or to miss or include relevant or unwanted pixels respectively. This effects can get worse as the noise increases. For this application however threshold method works well because after the application of a \texttt{contrast stretch filter} the analysed images presents high contrasts between the background and the bacteria (see figure \ref{fig:contrast}). Contrast stretch filter  stretches or scales the range of pixel values between an upper and lower limits. Pixel values that are above or below this range are saturated to the upper or lower limit value, respectively while piexels that lies in the interval are scaled according to the following formula: 
...
\begin{figure}
\centering
  \subcaptionbox{Origianal frame. The bacteria are represented by the small darker cluter. Light gray and white is noise and background. }[.4\linewidth]{
    \includegraphics[width=.425\linewidth]{./images/bacteriasmall}}\quad
  \subcaptionbox{This represents a segmented and inverted version of the image on the left. White cluster are bacteria.}[.4\linewidth][c]{%
    \includegraphics[width=.425\linewidth]{./images/bacteriasmall_threshold}}\quad
 \end{figure}
